...
with open(args.config_path, "r") as f:
    argdict = json.load(f)
    assert (
        argdict["sample_rate"] == args.sample_rate
...
if __name__ == "__main__":
    ...
    for i, (speaker_path, sentence) in enumerate(input_file):
        if args.spkr_embedding_path:
            i_wavs.append(
                os.path.join(
                    ...
                    os.path.basename(speaker_path)[:-4] + ".npy",
                )
            )
        else:
            ...
            i_wavs.append(audio)
        phones = (
            phonemizer(sentence.strip().lower(), lang="en_us")
            .replace("[", " ")
            .replace("]", " ")
            .split()
        )
        phones = [
            "".join(i for i in phone if not i.isdigit())
            for phone in phones
            if phone.strip()
        ]
        ...
        if len(i_wavs) == args.batch_size:
            print(
                f"Inferencing batch {written//args.batch_size+1}, total {len(input_file)//args.batch_size+1} baches."
            )
            synthetic = model(i_wavs, i_phones)
            for s in synthetic:
                sf.write(
                    os.path.join(args.outputdir, f"sentence-{written+1}-1.wav"),
                    s,
                    args.sample_rate,
                )
                written += 1
