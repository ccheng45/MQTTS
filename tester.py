...
class Wav2TTS_infer(nn.Module):
    def __init__(self, hp):
        ...
        self.phone_embedding = nn.Embedding(
            len(self.hp.phoneset),
            hp.hidden_size,
            padding_idx=self.hp.phoneset.index("<pad>"),
        )
        self.load()
        self.spkr_embedding = Inference("pyannote/embedding", window="whole")
        self.vocoder = Vocoder(
            hp.vocoder_config_path, hp.vocoder_ckpt_path, with_encoder=True
        )
    ...
    def forward(self, wavs, phones):
        ...
        with torch.no_grad():
            ...
            for wav in wavs:
                if self.hp.spkr_embedding_path:
                    ...
                    speaker_embedding = self.spkr_embedding(
                        {"waveform": wav, "sample_rate": self.hp.sample_rate}
                    )
            ...
            low_background_noise = (
                torch.randn(batch_size, int(self.hp.sample_rate * 5.0))
            ...
            for phone in phones:
                phone = [
                    self.hp.phoneset.index(ph)
                    for ph in phone
                    if ph != " " and ph in self.hp.phoneset
                ]
            ...
            for i, s in enumerate(synthetic):
                ...
                lengths.append(
                    len(s) * 256
                )  # Have to change according to vocoder stride!
                pad = (
                    base_prior[i, base_prior.size(1) // 2]
                    .unsqueeze(0)
                    .expand(to_pad, -1)
                )
                if self.hp.clean_speech_prior:
