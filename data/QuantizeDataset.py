...
class QuantizeDataset(data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        phonemes = [
            self.phoneset.index(ph)
            for ph in metadata["phoneme"].split()
            if ph in self.phoneset
        ]
        ...
        if self.hp.speaker_embedding_dir is None:
            ...
            speaker_embedding = os.path.join(
                self.hp.speaker_embedding_dir, os.path.splitext(_name)[0] + ".npy"
            )
        ...
        start, end = np.full(
            (1, self.hp.n_cluster_groups), self.hp.n_codes + 1, dtype=np.int16
        ), np.full((1, self.hp.n_cluster_groups), self.hp.n_codes, dtype=np.int16)
    ...
    def seqCollate(self, batch):
        ...
        for _, qs, qe, phone, _ in batch:
            # Deal with phonemes
            phone_mask = np.array(
                [False] * len(phone) + [True] * (max_phonelen - len(phone))
            )
            phone = np.pad(phone, [0, max_phonelen - len(phone)])
            # Deal with quantizations
            q_mask = np.array([False] * len(qs) + [True] * (max_len_q - len(qs)))
            qs = np.pad(
                qs, [[0, max_len_q - len(qs)], [0, 0]], constant_values=self.hp.n_codes
            )
            qe = np.pad(
                qe, [[0, max_len_q - len(qe)], [0, 0]], constant_values=self.hp.n_codes
            )
            # Aggregate
...
class QuantizeDatasetVal(QuantizeDataset):
    ...
    def __getitem__(self, i):
        speaker_embedding, quantization_s, quantization_e, phonemes, dataname = (
            super().__getitem__(i)
        )
        audio, sampling_rate = sf.read(dataname)
        audio = normalize(audio) * 0.95
