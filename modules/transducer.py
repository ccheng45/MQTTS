...
class ARCodeTransformer(nn.Module):
    def __init__(self, hp, n_decoder_codes):
        ...
        self.model = TransformerDecoder(
            nn.ModuleList(
                [
                    TransformerDecoderLayer(ar_hp, with_cross_attention=False)
                    for i in range(hp.ar_layer)
                ]
            )
        )
        self.embedding = nn.ModuleList(
            [
                nn.Embedding(n_decoder_codes, hp.ar_hidden_size)
                for _ in range(self.hp.n_cluster_groups - 1)
            ]
        )
        ...
        tgt_mask = (
            torch.tril(torch.ones(hp.n_cluster_groups, hp.n_cluster_groups), diagonal=0)
            == 0
        )
    ...
    def infer(self, cond, gt=None):
        ...
        ret, _, _, _ = self.model(
            cond, memory=None, tgt_mask=self.tgt_mask[: i + 1, : i + 1]
        )
...
class Transducer(nn.Module):
    def __init__(self, hp):
        ...
        self.n_decoder_codes = (
            hp.n_codes + 3 if self.hp.use_repetition_token else hp.n_codes + 2
        )
        self.embeddings = nn.ModuleList(
            [
                nn.Embedding(self.n_decoder_codes, per_embedding)
                for i in range(hp.n_cluster_groups)
            ]
        )
