...
class Wav2TTS(pl.LightningModule):
    def __init__(self, hp):
        ...
        self.phone_embedding = nn.Embedding(
            len(self.data.phoneset),
            hp.hidden_size,
            padding_idx=self.data.phoneset.index("<pad>"),
        )
    ...
    def val_dataloader(self):
        dataset = data.DataLoader(
            self.val_data, num_workers=self.hp.nworkers, shuffle=False
        )
        return dataset
    ...
    def training_step(self, batch, batch_idx):
        ...
        speaker_embedding = self.spkr_linear(
            F.dropout(speaker_embedding, self.hp.speaker_embed_dropout)
        )
        ...
        target = recons_segments["logits"][~batch["quantize_mask"]].view(
            -1, self.n_decode_codes
        )
    ...
    def on_validation_epoch_start(self):
        ...
        if self.hp.sample_num > 0:
            self.sample_idxs = (
                list(range(start_point, start_point + half))
                + np.random.randint(
                    low=start_point + half,
                    high=len(self.val_data),
                    size=self.hp.sample_num // 2,
                ).tolist()
            )
        else:
            self.sample_idxs = []
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx in self.sample_idxs:
            ...
            synthetic, infer_attn = self.TTSdecoder.inference_topkp_sampling_batch(
                phone_features, spkr, phone_mask, prior=None, output_alignment=True
            )
            ...
            sw.add_audio(
                f"generated/{batch_idx}",
                synthetic,
                self.global_step,
                self.hp.sample_rate,
            )
            sw.add_audio(
                f"vocoder-reconstructed/{batch_idx}",
                reconstructed_gt,
                self.global_step,
                self.hp.sample_rate,
            )
            sw.add_audio(
                f"groundtruth/{batch_idx}",
                ground_truth[0],
                self.global_step,
                self.hp.sample_rate,
            )
            ...
            # Plot attentions
            self.plot_attn(
                recons_segments["encoder_attention"], f"enc-attn/{batch_idx}", (10, 10)
            )
            self.plot_attn(
                recons_segments["decoder_attention"], f"dec-attn/{batch_idx}", (10, 10)
            )
            self.plot_attn(
                [recons_segments["alignment"]], f"train-alignment/{batch_idx}", (10, 10)
            )
            self.plot_attn(
                [infer_attn.unsqueeze(0)], f"infer-alignment/{batch_idx}", (10, 10)
            )

    def plot_attn(self, attns, prefix, figsize):
        nheads = attns[0].size(1)
        fig, axs = plt.subplots(
            len(attns), nheads, constrained_layout=True, figsize=figsize
        )
        ...
        for i, attn in enumerate(attns):  # Each layers
            ...
            for j, head_attn in enumerate(attn[0]):
                axs[i][j].matshow(
                    head_attn, aspect="auto", origin="lower", interpolation="none"
                )
                if i != 0 or j != 0:
                    axs[i][j].axis("off")
