...
def train(rank, a, h):
    ...
    optim_g = torch.optim.Adam(
        itertools.chain(
            generator.parameters(), encoder.parameters(), quantizer.parameters()
        ),
        h.learning_rate,
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            loss_gen_all = (
                loss_gen_s + loss_gen_f + loss_fm_s + loss_fm_f + loss_mel + loss_q * 10
            )
            ...
            if rank == 0:
                ...
                if steps % a.stdout_interval == 0:
                    ...
                    print(
                        "Steps : {:d}, Gen Loss Total : {:4.3f}, Loss Q : {:4.3f}, Mel-Spec. Error : {:4.3f}, s/b : {:4.3f}".format(
                            steps,
                            loss_gen_all,
                            loss_q,
                            mel_error,
                ...
                if steps % a.checkpoint_interval == 0 and steps != 0:
                    ...
                    save_checkpoint(
                        checkpoint_path,
                        {
                            ...
                            "encoder": (
                                encoder.module if h.num_gpus > 1 else encoder
                            ).state_dict(),
                            "quantizer": (
                                quantizer.module if h.num_gpus > 1 else quantizer
                            ).state_dict(),
                        },
                    )
                    ...
                    with torch.no_grad():
                        for j, batch in enumerate(validation_loader):
                            ...
                            val_err_tot += F.l1_loss(
                                y_mel[:, :, :i_size], y_g_hat_mel[:, :, :i_size]
                            ).item()
